...
@Component({
  ...
  styles: [`
    ...
    .d-avatar-btn {
      ...
      font-weight: 700;
      text-decoration: none;
      border: 2px solid rgba(201, 146, 13, .3);
  ...
  template: `
    ...
    <!-- ════════════ MOBILE (hidden on md+) ════════════ -->
    <div class="animate-fade md:hidden flex flex-col min-h-full bg-page">

      <!-- Topbar -->
      <div
        class="sticky top-0 z-20 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          <a routerLink="/profile"
             class="w-8 h-8 rounded-full bg-linear-to-br from-gold to-gold-dk flex items-center justify-center text-white text-xs font-bold no-underline border-2 border-[rgba(201,146,13,.3)] shadow-[0_2px_8px_rgba(201,146,13,.35)]">
            {{ auth.initials() }}
          </a>
        </div>
      </div>
      ...
      <div class="flex-1 px-4 py-5 pb-24">
        ...
        @if (disputeTotal() > 0) {
          <div class="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3.5 mb-5">
            ...
            <a routerLink="/disputes"
               class="shrink-0 text-xs font-semibold text-amber-700 no-underline hover:underline">{{ 'dashboard.viewDisputes' | translate }}
              →</a>
          </div>
        }
        ...
        @if (loading()) {
          @for (i of [1, 2, 3]; track i) {
            <div class="flex items-center gap-3 bg-white rounded-[18px] p-4 mb-2 shadow-[0_1px_4px_rgba(15,23,42,.06)]">
              ...
              <div class="flex-1">
                <div class="skeleton-shimmer rounded h-3 w-1/2 mb-1.5"></div>
                <div class="skeleton-shimmer rounded h-2.5 w-[30%]"></div>
              </div>
    ...
    <div class="animate-fade hidden md:flex flex-col min-h-full" style="background:#F5F6FA">
      ...
      <div class="d-main">
        ...
        <div class="animate-entry d-page-title">
        ...
        <!-- Content grid -->
        <div class="animate-entry d-content-grid">
